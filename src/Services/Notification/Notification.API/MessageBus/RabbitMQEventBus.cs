...
namespace Notification.API.MessageBus;
...
public class RabbitMQEventBus
{
    ...
    {
        SubscribeToEvent("account.accountcreated", HandleAccountCreatedEvent);
    ...
    {
        var queueName = $"notification_service_{routingKey}";
    ...
    private async Task HandleAccountCreatedEvent(string message)
    {
        using var scope = _serviceProvider.CreateScope();
        var handler = scope.ServiceProvider.GetRequiredService<AccountEventHandler>();
        var @event = JsonSerializer.Deserialize<AccountCreatedEvent>(message);
        if (@event != null)
            await handler.HandleAccountCreatedEvent(@event);
    }
